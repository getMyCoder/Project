...
function style() {
    ...
    $(".middle-map").css({
        "width": size.w(1000) + "px",
        "height": size.w(700) + "px",
        "top": size.h(0) + "px",
        "left": (HW.width - size.w(1000)) * 0.5 + "px"
    });
...
function scrollTable(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        createTr += "<tr style=\"height:" + sizeH + "px\">\n" +
            "    <td><span style='height: "+sizeH+"px;line-height: "+sizeH+"px'>" + data[i].attackToIp + "</span></td>\n" +
            "    <td><span style='height: "+sizeH+"px;line-height: "+sizeH+"px'>" + data[i].company + "</span></td>\n" +
            "    <td><span style='height: "+sizeH+"px;line-height: "+sizeH+"px'>" + data[i].industry + "</span></td>\n" +
            "    <td><span style='height: "+sizeH+"px;line-height: "+sizeH+"px'>" + data[i].attackIp + "</span></td>\n" +
            "    <td><span style='height: "+sizeH+"px;line-height: "+sizeH+"px'>" + data[i].time + "</span></td>\n" +
            "</tr>";
    }
    ...
    option = {
        tooltip: {
            ...
            formatter:function(params){
                return params[0].axisValue +'<br>'+ params[0].value+"次"
            }
        },
        legend: {
            show: false
...
function createCheck(data,callback) {
    var checkLable = "", allLable = "", title = "";
    for (var i = 0; i < data.length; i++) {
        title = "<div data-dictType='"+data[i].dictType+"' class=\"statistics-box-title statistics-box-title-M\">" + data[i].title + "</div>";
        checkLable = "";
        for (var j = 0; j < data[i].data.length; j++) {
            checkLable += "<h5 data-value='"+data[i].value[j]+"' data-dictType='"+data[i].dictType+"' style=\"width:" + (100 / data[i].column) + "%\">" +
                "<p>" +
                "<span>" +
                "<i>" +
...
// layui获取时间
function layuiGetTime(callback) {
    var getTimer={
        start:null,
        end:null
    }
